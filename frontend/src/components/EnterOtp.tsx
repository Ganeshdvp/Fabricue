    ...
    <>
      <div className="min-h-screen w-full flex items-center justify-center bg-[url(https://img.freepik.com/premium-photo/background-with-grip_1286621-341.jpg?semt=ais_rp_progressive&w=740&q=80)] bg-no-repeat bg-cover bg-center">
        <div className="flex flex-col items-center md:max-w-105.75 w-95 bg-white rounded-2xl shadow-lg p-6 sm:p-10">
          ...
          <p className="mt-2 text-sm text-gray-900/60 text-center">
            Enter the 6-digit verification code sent to your registered email to
            continue.
          </p>
          <p className="text-[12px] text-black font-semibold mt-2 text-center">
            Note:- This OTP will expire in 5 minutes. For your safety, do not
          ...
          <button
            ...
            className="mt-8 w-full px-6 max-w-80 h-11 rounded-full text-white text-sm bg-amber-500 hover:opacity-90 transition-opacity cursor-pointer"
          >
            {isPending ? <Loading color={"border-white"} /> : "Verify OTP"}
          </button>
          <p className="mt-4 text-[12px]">
            Didn’t receive the code?{" "}
            <span className="text-amber-500 font-semibold underline cursor-pointer">
              Resend OTP
            </span>
          </p>
        </div>
