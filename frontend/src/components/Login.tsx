...
export const Login = () => {
  ...
  // api call
  const {mutate:signUpMutate, isPending: signUpPending, isError: signUpErrorBoolean, error: signUpError} = useMutation({
    ...
    const {mutate:loginMutate, isPending: loginPending, isError: loginErrorBoolean, error: loginError} = useMutation({
    mutationFn: async (data)=>{
      ...
      <div className="flex justify-center mt-15">
        ...
        >
          <Form className="flex w-full flex-col items-center justify-center max-w-110 bg-gray-50 p-6 rounded-2xl shadow-xl">
            ...
            <p className="mt-3 text-sm text-gray-500/90 mb-12">
              {toggle
                ? "Welcome back! Please sign in to continue"
                : "Create you account to order the products!"}
            </p>
            {!toggle && (
              <div className="flex flex-col justify-start w-full">
                <div className="flex h-12 w-full items-center gap-2 overflow-hidden rounded-full border border-gray-200 bg-transparent pl-5 focus-within:border-gray-300">
                  ...
                  <Field
                    name="fullName"
                    placeholder={
                      role === "seller" ? "Seller Full Name" : "User Full Name"
                    }
                ...
                <ErrorMessage name="fullName">
                  {(msg) => (
                    <span className="text-[10px] text-amber-600 ml-4">{msg}</span>
                  )}
                </ErrorMessage>
              </div>
            )}
            <div className="flex flex-col justify-start w-full">
                <div className="flex h-12 w-full mt-2 items-center gap-2 overflow-hidden rounded-full border border-gray-200 bg-transparent pl-5 focus-within:border-gray-300">
              <svg
            ...
            <ErrorMessage name="email">
                {(msg) => (
                  <span className="text-[12px] text-amber-600 ml-4">{msg}</span>
                )}
              </ErrorMessage>
            </div>
            <div className="flex flex-col justify-start w-full">
              <div className="mt-2 flex h-12 w-full items-center gap-2 overflow-hidden rounded-full border border-gray-200 bg-transparent pl-5 focus-within:border-gray-300">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="18"
                height="18"
            ...
            >
              {toggle ? ( loginPending ? <Loading/> : 'Login') : ( signUpPending ? 'loading...' : 'Sign Up' )}
            </button>
            {toggle ? (
            ...
            <p className="mt-4 text-red-600 text-[13px]">{toggle ? (loginErrorBoolean ? loginError?.response?.data?.message : null) : (signUpErrorBoolean ? signUpError?.response?.data?.message : null)}</p>
          </Form>
        </Formik>
      </div>
    </>
